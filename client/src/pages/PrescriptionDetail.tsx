...
import { ArrowLeft, Printer, MessageSquare, Mail, Download, Loader2 } from "lucide-react";
...
export default function PrescriptionDetail() {
  ...
  const { prescriptions, patients, doctors, medicines, loading } = useStore();

  const r = prescriptions.find(x => x.id === id) ?? (id === 0 ? prescriptions[0] : undefined);
  const p = patients.find(x => x.id === r?.patientId);
  const d = doctors.find(x => x.id === r?.doctorId);
  const branch = BRANCHES.find(b => b.id === (p?.branchId ?? 1));
  const { toast } = useToast();

  /* Show spinner while loading — prevents false "not found" flash */
  if (!r && loading) {
    return (
      <PageContainer>
      ...
      <PageContainer>
        ...
        <Card className="p-10 text-center">
          ...
          <Link href="/prescriptions">
            <Button variant="outline" size="sm" className="mt-4">Return to prescriptions list</Button>
          </Link>
        </Card>
      </PageContainer>
    );
  ...
  /* Patient may be null if deleted — show graceful fallback */
  const patientName = p?.fullName ?? "Deleted Patient";
  const patientMeta = p ? `${p.mrn} · ${p.gender} · ${p.age}y · ${p.bloodGroup}` : "Record removed";
    ...
    <PageContainer>
      <div className="flex items-center justify-between mb-4 no-print">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => toast({ title: "Sent via WhatsApp", description: p ? `Sent to ${p.phone}` : "No patient phone on record" })}><MessageSquare className="h-4 w-4" /> WhatsApp</Button>
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => toast({ title: "Emailed prescription", description: p ? `Sent to ${p.email}` : "No patient email on record" })}><Mail className="h-4 w-4" /> Email</Button>
      ...
      <Card className="print-page mx-auto max-w-[860px] p-10 border-card-border bg-white text-black dark:bg-card dark:text-foreground">
        <div className="flex items-start justify-between border-b-2 border-primary pb-5">
          ...
          <div className="text-right text-[12px]">
            <div className="font-semibold text-[14px]">{d?.fullName ?? "Unassigned"}</div>
        ...
        <div className="grid grid-cols-3 gap-6 py-5 border-b border-border text-[12.5px]">
          <div>
            ...
            <div className="text-muted-foreground">{patientMeta}</div>
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-6 pt-5 border-t border-border text-[12px]">
          ...
          <div className="text-right">
            ...
            <div className="inline-block min-h-[64px]">
              <svg viewBox="0 0 220 60" className="w-[200px] h-[60px] text-primary">
                <path d="M10 40 C 30 10, 50 50, 70 30 S 110 10, 130 40 S 180 30, 210 20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" />
              </svg>
            </div>
            <div className="text-[12px] font-medium">{d?.fullName ?? "Unassigned"}</div>
        ...
        <div className="mt-6 pt-4 border-t border-border text-[10px] text-muted-foreground flex items-center justify-between">
          ...
          <span>Confidential · {new Date(r.createdAt).toLocaleDateString()}</span>
        </div>
      </Card>
    </PageContainer>
  );
